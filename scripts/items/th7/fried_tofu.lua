...
function FriedTofu:onBingeEaterChange(player, item, diff)
    if (player:HasCollectible(FriedTofu.Item)) then
        player:AddCacheFlags(CacheFlag.CAHCE_RANGE | CacheFlag.CACHE_LUCK);
        player:EvaluateItems();
    end
end
FriedTofu:AddCustomCallback(CLCallbacks.CLC_POST_CHANGE_COLLECTIBLES, FriedTofu.onBingeEaterChange, CollectibleType.COLLECTIBLE_BINGE_EATER);
...
FriedTofu:AddCustomCallback(CLCallbacks.CLC_POST_CHANGE_COLLECTIBLES, FriedTofu.postChangeCollectibles, FriedTofu.Item);

function FriedTofu:onEvaluateCache(player, flag) 
    if (player:HasCollectible(CollectibleType.COLLECTIBLE_BINGE_EATER)) then
        local tofuCount = player:GetCollectibleNum(FriedTofu.Item);
        if (flag == CacheFlag.CAHCE_RANGE) then
