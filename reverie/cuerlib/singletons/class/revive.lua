...
---@class ReviveInfo
---@field CanBorrowLife boolean @Should player revives from a life from the other twin if he's out of lives.
---@field BeforeVanilla boolean @Should this revive before vanilla revives.
---@field Animation string @Death animation the player should play.
---@field ReviveFrame integer @Revives after these frames.
---@field Callback ReviveCallback @A callback after revives. Params: (EntityPlayer player, EntityPlayer reviver)

local Mod = SINGLETON;

local Revive = Mod:NewClass();
...
do -- Check.
    ...
    function Revive:CanPlayerRevive(player)
        ...
        local callbacks = Isaac.GetCallbacks(Mod.Callbacks.CLC_PRE_REVIVE);
...
local function RevivePlayer(player)
    ...
    if (Mod.Players:IsOnlyRedHeartPlayer(playerType)) then
        if (maxHearts <= 0) then
    ...
    elseif (Mod.Players:IsOnlyBoneHeartPlayer(playerType)) then
        ...
        if (maxHearts > 0) then
            ...
            if (player:GetSoulHearts() <= 0) then
                Mod.Players:AddRawSoulHearts(player, 1);
            end
        end
    end
    ...
    Isaac.RunCallback(Mod.Callbacks.CLC_POST_REVIVE, player, info);
end
...
do -- Events.
    local function PostPlayerKilled(mod, entity)
        ...
        if (canRevive) then
            ...
            local onlyRedHearts = Mod.Players:IsOnlyRedHeartPlayer(playerType);
            local onlyBoneHearts = Mod.Players:IsOnlyBoneHeartPlayer(playerType);
            ...
            if (not shouldHasHeart) then
                ...
                if (soulHearts > 0) then
                    Mod.Players:AddRawSoulHearts(player, -soulHearts)
                end
            end
        end
    end
