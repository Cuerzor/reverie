...
local function GetGlobalData(init)
    local data = Mod:GetGlobalModData();
    if (init) then
...
local function PostUpdate(mod)
    ...
    if (game:IsGreedMode()) then
        ...
        if (wave > data.LastGreedWave) then
            Isaac.RunCallback(Mod.Callbacks.CLC_POST_NEW_GREED_WAVE, wave);
        end
        ...
        if (clear and clear ~= data.Cleared) then
            ...
            if (wave == bossWave or wave == totalWave) then
                ...
                Isaac.RunCallback(Mod.Callbacks.CLC_POST_GREED_WAVE_END, state);
            end
        end
