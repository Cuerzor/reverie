...
local Collectibles = Mod:NewClass()

local function PostPickupCollectible(player, item, touched)
    touched = touched or false;
    Isaac.RunCallbackWithParam(Mod.Callbacks.CLC_POST_PICK_UP_COLLECTIBLE, item, player, item, touched);
end

local function PostPickupTrinket(player, trinket, golden, touched)
    touched = touched or false;
    Isaac.RunCallbackWithParam(Mod.Callbacks.CLC_POST_PICK_UP_TRINKET, trinket, player, trinket, golden, touched);
end
...
local function PostGainCollectible(player, item, count, touched, holding)
    ...
    Isaac.RunCallbackWithParam(Mod.Callbacks.CLC_POST_GAIN_COLLECTIBLE, item, player, item, count, touched, holding);
end

local function PostLoseCollectible(player, item, count)
    count = count or 1;
    Isaac.RunCallbackWithParam(Mod.Callbacks.CLC_POST_LOSE_COLLECTIBLE, item, player, item, count);
end

local function PostChangeCollectibles(player, item, diff)
    Isaac.RunCallbackWithParam(Mod.Callbacks.CLC_POST_CHANGE_COLLECTIBLES, item, player, item, diff);
end
...
local function GetPickupData(pickup, init) 
    local data = Mod:GetEntityModData(pickup, true);
    if (data) then
...
local function GetPlayerData(player, init)
    local entData = Mod:GetEntityModData(player, true);
    if (init) then
...
function Collectibles.IsAnyHasCollectible(item, onlyTrue)
    ...
    for index, player in Mod.Players.PlayerPairs() do
...
function Collectibles:onGameStarted(isContinued)
    if (isContinued) then
        for p, player in Mod.Players.PlayerPairs()  do
