...
function ItemPools:EvaluateRoomBlacklist()
    ...
    for i = 1, collectibles.Size do
        ...
        if (config) then
            local callbacks = Isaac.GetCallbacks(Mod.Callbacks.CLC_EVALUATE_POOL_BLACKLIST);
...
local function PreGetCollectible(mod, pool, decrease, seed)
    ...
    local callbacks = Isaac.GetCallbacks(Mod.Callbacks.CLC_PRE_GET_COLLECTIBLE);
