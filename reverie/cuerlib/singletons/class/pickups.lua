...
local function PostCollectPickup(player, pickup)
    ...
    Isaac.RunCallbackWithParam(Mod.Callbacks.CLC_POST_PICKUP_COLLECTED, pickup.Variant, player, pickup);
end

function Pickups:GetPickupData(pickup) 
    local data = Mod:GetEntityModData(pickup);
...
function Pickups.CanCollect(player, pickup)
    ...
    return Isaac.RunCallbackWithParam(Mod.Callbacks.CLC_CAN_PICKUP_COLLECT, pickup.Variant, player, pickup);
end
            ...
            for p, player in Mod.Players.PlayerPairs() do
                for slot = 0, 3 do
                    ...
                    if (card == pickup.SubType) then
                        Isaac.RunCallbackWithParam(Mod.Callbacks.CLC_POST_PICK_UP_CARD, card, player, card);
                        return;
                    end
                end
            end
