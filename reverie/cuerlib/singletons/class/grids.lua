...
do -- GridData.
    ...
    local function GetData(create)
        local data = Mod:GetGlobalModData(true);
    ...
    if (Grids:IsRock(prevType)) then
        if (prevData.State == 1) then
            ...
            if ((newRock and gridData.State ~= 1) or newType == GridEntityType.GRID_STAIRS) then
                Isaac.RunCallbackWithParam(Mod.Callbacks.CLC_POST_GRID_DESTROYED, prevData.Type, prevData);
            end
        end
    end
end
...
function Grids:PostUpdate()
    ...
    for x = 0, width do
        for y = 0, height do
            ...
            Isaac.RunCallbackWithParam(Mod.Callbacks.CLC_POST_GRID_UPDATE, index, index, prevData, gridData);
...
function Grids:PostTearUpdate(tear)
    if (tear:IsDead()) then
        ...
        Isaac.RunCallbackWithParam(Mod.Callbacks.CLC_POST_GRID_DESTROYED, gridData.Type, gridData);
    end
end
