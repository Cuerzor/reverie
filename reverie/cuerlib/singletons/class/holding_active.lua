...
local HoldingActive = Mod:NewClass();
...
function HoldingActive:GetPlayerData(player, init)
    ...
    local playerData = Mod:GetEntityModData(player, true);
    if (init) then
...
function HoldingActive:ReleaseActive(player, item, ...)
    ...
    local callbacks = Isaac.GetCallbacks(Mod.Callbacks.CLC_RELEASE_HOLDING_ACTIVE);
...
function HoldingActive:ReleaseOnShoot(player, item)
    if (HoldingActive:GetHoldingItem(player) == item) then
        local shooting = Mod.Inputs.GetRawShootingVector(player);
...
local function PostNewRoom(mod)
    for p, player in Mod.Players.PlayerPairs(true) do
        if (HoldingActive:GetHoldingItem(player) > 0) then
            EndHolding(player);
        end
    end
